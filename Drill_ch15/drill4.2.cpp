...
{
    if (aa < 0 || 150 < aa) error("Person(): invalid age");	//ellenörzés a korra
    
    string nn = ff + ll;    // Egyszerre mindkét nevet ellenörzi, ezek a karakterek errort dobank
    for (char c : nn) {
        switch(c) {
...
try {
	...
	cout << "Enter some names and ages. Terminate with name 'end'" << endl;
	cout << "For example: "<< p.first_name() << ' ' << p.last_name() << ' ' << p.age() 
	     << endl;
